...
<template>
  <div
    class="grid gap-4 p-10 border rounded-2xl shadow-2xl text-center"
    :class="[
      hasCorrectAnswer
  ...
  >
    <h2 class="text-6xl font-medium text-slate-700 mb-6">{{ card.front }}</h2>
    <div class="mb-6">
      <input
        v-if="showBack"
        ref="inputEl"
        class="w-full text-6xl text-slate-700 text-center font-medium bg-slate-200 p-2 rounded-xl outline-none"
        :value="card.back"
        readonly
      />
      <input
        v-else
        ref="inputEl"
        class="w-full text-6xl text-slate-700 text-center font-medium bg-slate-200 p-2 rounded-xl focus:outline-8"
        :class="[
          hasCorrectAnswer
